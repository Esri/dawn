...
cd temp_dawn_build
/usr/local/rtc/cmake/3.29.2/bin/cmake ../dawn -DDAWN_BUILD_TESTS=OFF -DTINT_BUILD_TESTS=OFF -DDAWN_ENABLE_VULKAN=OFF -DTINT_BUILD_GLSL_WRITER=OFF -DTINT_BUILD_GLSL_VALIDATOR=OFF -DDAWN_USE_GLFW=OFF -DDAWN_BUILD_SAMPLES=OFF -DPython3_EXECUTABLE=/usr/local/rtc/python/3.12/bin/python3.12 -DENABLE_NULL=OFF -DDAWN_FETCH_DEPENDENCIES=OFF
cd ../dawn
touch "temp_file"
